...
class VoxelGrid:
    ...
    def voxelize(self):
        ...
        """
        idx = [tuple(this_idx) for this_idx in ((self.points + self.offset) / self.mesh_size).astype(int)]
        ...
        if len(self.points) > 1000000:
            for i in trange(len(self.points), desc='Building voxel grid'):
                sorted_points[tuple(idx[i])].append(i)
        else:
    ...
    def indices(self, voxels=None, merge=False):
        ...
        """
        if isinstance(voxels, tuple) and not isinstance(voxels[0], tuple):
    ...
    def occupied(self):
        ...
        """
        return list(self.sorted_points.keys())

    def is_occupied(self, idx):
        return idx in self.sorted_points
